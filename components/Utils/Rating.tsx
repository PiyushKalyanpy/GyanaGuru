const Rating = ({ value, noOfVotes }: any) => {
    ...
    const renderStar = ( index :any ) => {
      if (index < filledStars) {
        return (
          <span key={index} className="w-4 h-4 rounded-full  inline-block mr-1">
            <svg
        ...
        return (
          <span
            key={index}
            className="w-4 h-4 rounded-full inline-block mr-1 overflow-hidden"
          >
    ...
    return <div className="flex">{renderStars()}
            {/* showing value and no of votes */}
  
            <span className="text-zinc-500 text-sm ml-1">{value} · {noOfVotes} votes</span>
